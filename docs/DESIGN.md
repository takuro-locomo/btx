## 患者向け前後比較の見やすさ（2026-09-08）

- 短い導入文・選択中の部位の直後に顔の比較を表示。「気になるお悩みを選ぶ」は顔のすぐ下の横スクロールに配置し、症状変更でページを強制スクロールしない。
- 「変化が少ない例」は削除。期待できる変化と副作用の例を残す。
- 患者向けの施術後の説明図では、選んだ部位のしわ線と顎の凹凸線を省略して差を明確化。図の説明で省略を明記し、仕上がりを保証しない。元の臨床モードや用量ロジックには適用しない。

- 初期表示・症状変更時ともに顔全体を同じ位置・倍率で前後に表示。任意の「部位を拡大」操作も残す。目尻は通常片側を拡大し、閉じにくさの例では左右が見える範囲を両方の図に適用する。
- 「並べて比較」と「同じ位置で切り替え」を選べる。切替は表示する図だけを変え、料金や作用例に影響しない。症状変更時は顔全体表示・施術後側へ戻す。
- 注射範囲のピンクは初期状態では重ねない。「打つ範囲を重ねる」で両方に同じ範囲を表示する。しわの濃さと範囲の着色を混同させない。
- 目尻の通常比較でまぶたを強く閉じていた描画を修正。前後で自然な目の開きを共通にし、目の外側のしわを比較する。閉じようとしたときの左右差は明示した副作用の例でのみ示す。
- ガミースマイルは口角と口を開く表現を前後で共通にし、上唇の挙上と歯ぐきの見え方を変える。治療後に笑わなくなる図として示さない。
- 線の位置・太さ・濃さ、眼裂・口唇の描画座標は説明図の表示値。臨床量、改善率、副作用確率、個別の予測値ではない。医学情報・料金・用量に関する仕様は変更しない。
